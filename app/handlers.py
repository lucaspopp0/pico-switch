...
def _new_request(key: str, on_success=lambda: None, on_failure=lambda: None):
    path = 'remote-press?remote=' + request.urlencode(
        config.value["name"]) + '&button=' + str(key)
...
def on_dial_long_rgbled(routine: deprecated.Routine):
    ...
    req = _new_request(routine.name + "-long", on_success_rgbled,
                       on_failure_rgbled)
