...
class TestApiServer(unittest.TestCase):
    ...
    def test_start(self, mock_socket_module):
        ...
        mock_socket_module.socket.assert_called_once_with(
            mock_socket_module.AF_INET, mock_socket_module.SOCK_STREAM)
        mock_sock.setsockopt.assert_called_once_with(
            mock_socket_module.SOL_SOCKET, mock_socket_module.SO_REUSEADDR, 1)
    ...
    def test_send_no_connection(self):
        ...
        self.assertIn("Can't send response, no connection instance",
    ...
    def test_internal_error_default_handler_with_print_exception(
            self, mock_sys):
        srv = server.Server()
        mock_connection = Mock()
        srv._connect = mock_connection
